...
router.get("/contact",async function(req,res){
    ...
    res.render("admin/patient_messages.ejs",obj)
})
...
router.get("/servicepage",async function(req,res){
    var servicepage = await exe (`SELECT * FROM servicepage`);
    var obj = {"servicepage":servicepage}
    res.render("admin/servicepage.ejs",obj)
})

router.post("/save_servicepage",async function(req,res){
    var d = req.body;
    var sql = `INSERT INTO servicepage(heading,description )VALUES(?,?)`;
    var data = await exe (sql,[d.heading,d.description ]);
    res.redirect("/admin/servicepage")
})

router.get("/delete_servicepage/:id",async function(req,res){
    var id = req.params.id;
    var servicepage = await exe (`SELECT * FROM servicepage`);
    var data = await exe (`DELETE FROM servicepage WHERE servicepage_id = '${id}'`);
    res.redirect("/admin/servicepage")

})

router.get("/edit_servicepage/:id",async function(req,res){
    var id = req.params.id;
    var servicepage = await exe (`SELECT * FROM servicepage WHERE servicepage_id = '${id}'`);
    var obj = {"servicepage":servicepage[0]}
    res.render("admin/edit_servicepage.ejs",obj)

})

router.post("/save_edit_servicepage",async function(req,res){
    var d = req.body;
    var sql = await exe(`UPDATE servicepage SET heading ="${d.edit_heading}", description  ="${d.edit_description}" WHERE servicepage_id = "${d.edit_servicepage_id}"`);
    res.redirect("/admin/servicepage")
})


//CREATE TABLE servicepage(servicepage_id INT PRIMARY KEY AUTO_INCREMENT , heading VARCHAR(200), description VARCHAR(500))
...
router.post("/save_gallery",async function(req,res){
    ...
    var d = req.body;
    var sql = `INSERT INTO gallery (gallery_image, gallery_name, gallery_info)VALUES(?,?,?)`;
    await exe (sql,[gallery_image,d.gallery_name,d.gallery_info]);
